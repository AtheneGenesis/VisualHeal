...
VisualHeal.LibHealComm = LibStub("LibHealComm-4.0");
...
function VisualHeal:UnitGetIncomingHealsBefore(aUnit, endtime)

	if not aUnit then
		return 0;
	end

	if self.LibHealComm then
		local tTargetGUID = UnitGUID(aUnit);
		return ((self.LibHealComm:GetOthersHealAmount(tTargetGUID, self.LibHealComm.ALL_HEALS, endtime) or 0) * (self.LibHealComm:GetHealModifier(tTargetGUID) or 1));
	else
		return 0;
	end
end

function VisualHeal:UnitGetIncomingHeals(aUnit, aCasterUnit)
	...
	if self.LibHealComm then
		...
		if aCasterUnit then
			...
			return (self.LibHealComm:GetHealAmount(tTargetGUID, self.LibHealComm.ALL_HEALS, nil, tCasterGUID) or 0) * (self.LibHealComm:GetHealModifier(tTargetGUID) or 1);
		else
			return (self.LibHealComm:GetHealAmount(tTargetGUID, self.LibHealComm.ALL_HEALS) or 0) * (self.LibHealComm:GetHealModifier(tTargetGUID) or 1);
		end
	else
		return 0;
	end
...
function VisualHeal:IncomingBarUpdate()
	...
	local healTime, healFrom, healAmount = self.LibHealComm:GetNextHealAmount(UnitGUID("player"), self.LibHealComm.DIRECT_HEALS)
	
	if healTime then
	frame.text:SetText(string.format("%.1fs + %d", (healTime - GetTime()), healAmount));
    frame.text:SetTextColor(230, 230, 0);
	end
	
    -- Update incoming heal
    local heal = self:UnitGetIncomingHeals("player") or 0;
    local playerHeal = self:UnitGetIncomingHeals("player", "player") or 0;
...
function VisualHeal:BarUpdate(frame)
	...
	local _,_,_,startTime,endTime,_,_,_,_ = CastingInfo()
	local casttime
	if startTime then
	...
	-- Get Healing before player heal
	local BeforeplayerHeal = self:UnitGetIncomingHealsBefore(frame.unit, GetTime() + casttime) or 0;
	frame.beforeplayerHeal:SetPoint("BOTTOMRIGHT", frame.health, "BOTTOMRIGHT", width * BeforeplayerHeal / healthMax, 0);
	frame.beforeplayerHeal:SetVertexColor(0.25, 0.45, 0.25, 0.7)
	
    -- Update player's heal
    local playerHeal = self:UnitGetIncomingHeals(frame.unit, "player") or 0;
    frame.playerHeal:SetPoint("BOTTOMRIGHT", frame.beforeplayerHeal, "BOTTOMRIGHT", width * playerHeal / healthMax, 0);
    local waste = 0;
    if (playerHeal > (healthMax - (health + BeforeplayerHeal))) then
        waste = (playerHeal - (healthMax - (health + BeforeplayerHeal))) / playerHeal;
    end
    ...
    -- Update other's heal
    local othersHeal = (self:UnitGetIncomingHeals(frame.unit) or 0);
    frame.othersHeal:SetPoint("BOTTOMRIGHT", frame.health, "BOTTOMRIGHT", width * othersHeal / healthMax, 0);
end
...
function VisualHeal:BarConfigure(frame)
    ...
    if (not frame.isIncoming) then
		...
		frame.beforeplayerHeal:SetTexture(barTexture);
    end
    ...
    if (frame.isIncoming) then
		...
		frame.sparkb:SetHeight(height * 1.5);
        local position = self.db.profile.OutgoingPosition;
        frame:ClearAllPoints();
        frame:SetPoint(position.point, position.parent, position.relpoint, position.x, position.y);
    end
...
function VisualHeal:BarCreate(isIncoming)
	...
	if (not isIncoming) then
        frame.beforeplayerHeal = frame:CreateTexture(nil, "BORDER")
        frame.beforeplayerHeal:ClearAllPoints()
        frame.beforeplayerHeal:SetPoint("TOPLEFT", frame.health, "TOPRIGHT", 0, 0)
		frame.beforeplayerHeal:SetVertexColor(0.25, 0.45, 0.25, 0.7)
    end
    ...
    if (not isIncoming) then
        ...
        frame.playerHeal:SetPoint("TOPLEFT", frame.beforeplayerHeal, "TOPRIGHT", 0, 0)
    end

    frame.othersHeal = frame:CreateTexture(nil, "BACKGROUND")
	...
	if (not isIncoming) then
	frame.sparkb = frame:CreateTexture(nil, "OVERLAY");
    frame.sparkb:SetTexture("Interface\\CastingBar\\UI-CastingBar-Spark");
    frame.sparkb:SetVertexColor(1, 1, 1, 1)
    frame.sparkb:SetBlendMode('ADD');
    frame.sparkb:SetWidth(10);
    frame.sparkb:SetPoint("CENTER", frame.beforeplayerHeal, "RIGHT", 0, 0);
	end
...
function VisualHeal:OnInitialize()
	...
	if self.LibHealComm then
		self.LibHealComm.RegisterCallback(self, "HealComm_HealStarted", "UNIT_HEAL_PREDICTION");
		self.LibHealComm.RegisterCallback(self, "HealComm_HealStopped", "UNIT_HEAL_PREDICTION");
		self.LibHealComm.RegisterCallback(self, "HealComm_HealDelayed", "UNIT_HEAL_PREDICTION");
		self.LibHealComm.RegisterCallback(self, "HealComm_HealUpdated", "UNIT_HEAL_PREDICTION");
	end
end
...
local checkList = {};
function VisualHeal:GROUP_ROSTER_UPDATE()
    for i, frame in ipairs(outgoingList) do
...
function VisualHeal:UnitUpdate(unit, endtime)
    if (not self.db.profile.ShowOutgoing) then return end
    local playerHeal = self:UnitGetIncomingHeals(unit, "player") or 0;
    local unitGUID = UnitGUID(unit);
    local frame = outgoingTable[unitGUID];
...
function VisualHeal:UNIT_HEAL_PREDICTION(aEvent, aCasterGUID, aSpellID, aHealType, aEndTime, ...)

	local tTargets = { n = select("#", ...), ... };

	for i = 1, table.getn(tTargets) do
		unit = (self.LibHealComm.guidToUnit[tTargets[i]] or nil)
		if (self.db.profile.ShowIncoming) then
			self:IncomingBarUpdate();
		end
		self:UnitUpdate(unit, aEndTime);
	end
end
